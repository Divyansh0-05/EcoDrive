    ...
    <div className="flex-1 h-screen bg-background flex flex-col md:flex-row overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col max-w-5xl mx-auto w-full p-4 md:p-8 h-full">
        ...
        <div className="mb-6">
          ...
          <p className="text-muted-foreground mt-2 text-sm">Powered by advanced telemetry analysis.</p>
        </div>

        {/* Chat Window */}
        <div className="flex-1 glass-card rounded-2xl flex flex-col overflow-hidden border border-border shadow-xl bg-[#0a0f0d]/80 relative z-10">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[80%] rounded-2xl p-5 ${
                  msg.role === 'user' 
                    ? 'bg-eco-glow/10 border border-eco-glow/20 text-foreground rounded-tr-sm' 
                  ...
                  <span className={`text-[10px] mt-2 block ${msg.role === 'user' ? 'text-eco-glow/50 text-right' : 'text-muted-foreground'}`}>
                    {msg.timestamp}
                  </span>
                </div>
            ...
            {isTyping && (
              <div className="flex gap-4 justify-start">
                ...
                <div className="bg-card border border-border rounded-2xl rounded-tl-sm p-5 flex items-center gap-2">
          ...
          <div className="p-4 bg-card border-t border-border">
            <form onSubmit={handleSend} className="relative flex items-center">
              <input
                ...
                className="w-full bg-card border border-border rounded-xl py-4 pl-5 pr-14 text-foreground placeholder-muted-foreground focus:outline-none focus:border-eco-glow/50 transition-colors"
                disabled={isTyping}
              />
              <button
                type="submit"
      ...
      <div className="hidden xl:flex w-[320px] bg-card border-l border-border p-6 flex-col z-10">
        ...
        <div className="space-y-4">
          <div className="p-4 bg-card border border-border rounded-xl">
            <span className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Avg Eco Score</span>
            <div className="text-2xl font-black text-foreground mt-1">{contextData?.avgEcoScore || '--'}</div>
          </div>
          ...
          <div className="p-4 bg-red-500/5 border border-red-500/10 rounded-xl">
            ...
            <div className="text-2xl font-black text-foreground mt-1">{contextData?.totalBrakes ?? '--'}</div>
          </div>
          ...
          <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl">
            ...
            <div className="text-2xl font-black text-foreground mt-1">{contextData?.totalTrips ?? '--'}</div>
          </div>
        </div>
