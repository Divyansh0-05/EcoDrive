  ...
  return (
    <div className="flex-1 min-h-screen bg-background p-8 overflow-y-auto font-sans relative select-none">
      ...
      <div className="mb-10">
        ...
        <p className="text-gray-400 text-sm mt-1">Driver XP rank leaderboards updated globally.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 max-w-[900px] mx-auto items-end">
        {/* 2nd Place */}
        {podium[1] && (
          <div className="glass-card rounded-2xl p-6 flex flex-col items-center relative border-white/5 order-2 md:order-1 h-[220px] justify-center">
            ...
            <div className="w-14 h-14 rounded-full bg-eco-bg border border-white/10 flex items-center justify-center text-lg font-bold text-white mb-3">
              {podium[1].name[0]}
            </div>
            <span className="text-sm font-extrabold text-white text-center truncate w-full">{podium[1].name}</span>
            <span className="text-[10px] text-eco-glow uppercase tracking-wider font-semibold mt-1">Level {podium[1].level}</span>
            <span className="text-xs font-bold text-gray-400 mt-2">{podium[1].total_xp} XP</span>
          </div>
        )}
        ...
        {podium[0] && (
          <div className="glass-card rounded-2xl p-8 flex flex-col items-center relative border-eco-glow/20 shadow-glow-green-lg order-1 md:order-2 h-[260px] justify-center bg-eco-bg/25">
            ...
            <div className="w-16 h-16 rounded-full bg-eco-bg border border-eco-glow/30 flex items-center justify-center text-xl font-bold text-white mb-3 shadow-glow-green">
              {podium[0].name[0]}
            </div>
            <span className="text-base font-extrabold text-white text-center truncate w-full">{podium[0].name}</span>
            <span className="text-xs text-eco-glow uppercase tracking-wider font-extrabold mt-1">Level {podium[0].level}</span>
            <span className="text-sm font-black text-white mt-2">{podium[0].total_xp} XP</span>
          </div>
        )}

        {/* 3rd Place */}
        {podium[2] && (
          <div className="glass-card rounded-2xl p-6 flex flex-col items-center relative border-white/5 order-3 h-[200px] justify-center">
            ...
            <span className="text-sm font-extrabold text-white text-center truncate w-full">{podium[2].name}</span>
            <span className="text-[10px] text-eco-glow uppercase tracking-wider font-semibold mt-1">Level {podium[2].level}</span>
            <span className="text-xs font-bold text-gray-400 mt-2">{podium[2].total_xp} XP</span>
          </div>
        )}
      </div>
          ...
          {remaining.map((user) => (
            <div key={user.id} className="px-6 py-4 flex items-center justify-between hover:bg-white/[0.01] transition-colors">
              <div className="flex items-center gap-4">
                <span className="text-xs font-bold text-gray-500 w-6 text-center">{user.rank}</span>
                <div className="w-9 h-9 rounded-xl bg-black/40 border border-white/5 flex items-center justify-center text-xs font-bold text-white">
                  {user.name[0]}
                </div>
                <div>
                  <span className="text-sm font-bold text-white block">{user.name}</span>
                  <span className="text-[10px] text-gray-500 font-medium uppercase mt-0.5 block">Level {user.level} • {user.eco_level}</span>
                </div>
              </div>

              <div className="flex gap-16 items-center pr-4">
                <span className="text-xs font-semibold text-gray-400 w-20 text-right">
